...
function rangeslider() {
    ...
    a = document.getElementById("rail");
    b = window.getComputedStyle(a, null).getPropertyValue("width");
    ...
    Slider.prototype.init = function () {
        ...
        if (this.domNode.previousElementSibling) {
            ...
            this.railMin = parseInt((this.minDomNode.getAttribute('aria-valuemin'))) + (this.thumbWidth);
        }
        else {
            this.railMin = parseInt((this.domNode.getAttribute('aria-valuemin'))) + (this.thumbWidth);
        };

        if (this.domNode.nextElementSibling) {
            this.maxDomNode = this.domNode.nextElementSibling;
            this.railMax = parseInt((this.maxDomNode.getAttribute('aria-valuemax'))) - (this.thumbWidth);
        }

        else {
            this.railMax = parseInt((this.domNode.getAttribute('aria-valuemax'))) - (this.thumbWidth);
        }

        this.valueNow = parseInt((this.domNode.getAttribute('aria-valuenow')));
        this.railWidth = parseInt(b.slice(0, -2) );
    ...
    Slider.prototype.moveSliderTo = function (value) {
        ...
        if (this.labelDomNode) {
            ...
            if (this.domNode.classList.contains('min')) {
                document.getElementById("minprice").value  = this.dolValueNow.toString();
            }
    
            if (this.domNode.classList.contains('max')) {
                document.getElementById("maxprice").value  = this.dolValueNow.toString();
            }
        }
    };
...
function plusSlides(n) {
  if (n == 1) {
    var i;
    slides[slideIndex - 1].className = "slideimg slideOut";
    slideIndex += n;
    if (slideIndex > slides.length) { slideIndex = 1 }
    slides[slideIndex - 1].className = "slideimg slideIn";
  }

  if (n == -1) {
    var i;
    slides[slideIndex - 1].className = "slideimg slideOutR";
    slideIndex += n;
    if (slideIndex < 1) { slideIndex = slides.length }
    slides[slideIndex - 1].className = "slideimg slideInR";
  }
