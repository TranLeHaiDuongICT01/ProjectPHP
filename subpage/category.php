...
<ul class="pagination justify-content-center">
    <!-- PREV BUTTON -->
    <li class="page-item" > <a  style="outline: none !important; border-radius:0 !important;" class="page-link" id="pageprev" href="<?= $link ?>&pagenumber=<?= $current_page - 1 ?>">Prev</a> </li>

    <!-- PAGES NUMBER BUTTON -->
    <?php
    if ($i % $num_products_on_each_page == 0) {
        $total_pages = (int)($i / $num_products_on_each_page);
    } else {
        $total_pages = (int)($i / $num_products_on_each_page) + 1;
    }
    $j = 1;
    while ($j <= $total_pages) {
        if ($j == $current_page) { ?>
            <li class="page-item"><a  style="color:white !important; background-color:#ffb0bd !important;outline: none!important; border-radius:0 !important;" class="page-link"><?= $j ?></a></li>
        <?php } else {
        ?>
            <li class="page-item" ><a style="outline: none !important;" class="page-link" href="<?= $link ?>&pagenumber=<?= $j ?>"><?= $j ?></a></li>
    <?php  }
        $j += 1;
    }  ?>
    <!-- NEXT BUTTON -->
    <li class="page-item"> <a style="outline: none !important; border-radius:0 !important;" class="page-link" id="pagenext" href="<?= $link ?>&pagenumber=<?= $current_page + 1 ?>">Next</a> </li>
</ul>
<!-- ----------- -->
