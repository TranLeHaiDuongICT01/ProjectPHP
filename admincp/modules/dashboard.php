            ...
            <?php foreach ($slides as $slide) { ?>
                <tr>
                    <form action="" method="POST">
                        ...
                        <td><input onclick="return confirm('Do you really want to delete this slide image?');" class="btn btn-danger btn-hover" type="submit" name="removeslide" value="REMOVE"></td>
                    </form>
                </tr>
            <?php } ?>
        </table>
...
if (isset($_POST['delete'])) {
    $stmt = $pdo->prepare("DELETE FROM tbl_admin WHERE id = ?");
    $stmt->bindParam(1, $_POST['delete'], PDO::PARAM_INT);
    $stmt->execute(); 
}
$sql_list = $pdo->prepare("SELECT * FROM tbl_admin ORDER BY username DESC");
$sql_list->execute();
  ...
  ?>
    <tr>
      ...
      <td><?php if ($user['role'] == 0) {
            ...
            echo 'PRODUCTS MANAGER';
          }elseif($user['role'] == 2) {
            echo 'INVOICES MANAGER';
          } ?>
      </td>
      <td>
          <form action="" method="post">
        <button class="btn btn-hover btn-outline-danger" type="subbmit" onclick="return confirm('Do you really want to delete this user?');" name="delete" value="<?php echo $user['id'] ?>">DELETE</button>
        </form>
      </td>
    </tr>
  <?php
